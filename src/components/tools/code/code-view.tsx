...
const CodeView = observer(
  ({ code, fileName, theme, changeCode }: { code: string; theme?: string; fileName: string; changeCode: Function }) => {
    const { app } = useStores();
    const th = useTheme();
    ...
    return (
      <div ref={refContainer} className="code-view code-view-light h-full w-full">
        <CodeMirror
