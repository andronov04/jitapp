...
const WorkbenchView = observer(({ workbench }: { workbench: IWorkbenchStore }) => {
  const previewUrl = "";
    ...
    <div key={workbench.id} className=" h-[600px] flex flex-col w-full">
      ...
      <div className="bg-secondary overflow-hidden w-full flex-grow rounded-lg">
        ...
        <Tabs defaultValue="code" className="w-full h-full">
          <TabsList>
            {workbench.tools?.map(item => <TabsTrigger key={item.id} value={item.id}>{item.name}</TabsTrigger>)}
          ...
          {workbench.tools?.map(item =>  <TabsContent style={{
            height: "calc(100% - 44px)",
          }} value={item.id} key={item.id} className="w-full">
